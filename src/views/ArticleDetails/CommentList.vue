<template>
  <div>
    <!-- 评论列表 -->

    <div
      class="cmt-list"
      :class="{
        'art-cmt-container-1': isShowCmtBox,
        'art-cmt-container-2': !isShowCmtBox
      }"
    >
      <!-- 2.van-list包裹列表，组件内源码，检测网页滚动事件，判断滚动位置是否快到达内容高度(触底)
    ->触发load事件执行 ->loading:为true -> 底部加载中文字出现 -> 内部认为现在处于加载中状态 -->
      <!-- 3.immediate-check作用：让list组件不要上来就检查是否触底
      原因：list中列表内容是异步网络请求回来的，标签挂载时，没有高度，list就会判定触底执行一次onLoad方法。
      问题：created发一次，onLOad立刻又发送一次，请求两次第一页数据/上来就请求第一页和第二页
       -->
      <van-list
        v-model="loading"
        :finished="finished"
        finished-text="没有更多了,快来发表评论吧"
        @load="onLoad"
        :immediate-check="false"
      >
        <!-- 评论的 Item 项 -->
        <div class="cmt-item" v-for="obj in commentArr" :key="obj.com_id">
          <!-- 头部区域 -->
          <div class="cmt-header">
            <!-- 头部左侧 -->
            <div class="cmt-header-left">
              <img :src="obj.aut_photo" alt="" class="avatar" />
              <span class="uname">{{ obj.aut_name }}</span>
            </div>
            <!-- 头部右侧 -->
            <div class="cmt-header-right">
              <van-icon
                name="like"
                size="16"
                color="red"
                v-if="obj.is_liking === true"
                @click="likeClickFn(true, obj)"
              />
              <van-icon
                name="like-o"
                size="16"
                color="gray"
                v-else
                @click="likeClickFn(false, obj)"
              />
            </div>
          </div>
          <!-- 主体区域 -->
          <div class="cmt-body">
            {{ obj.content }}
          </div>
          <!-- 尾部区域 -->
          <div class="cmt-footer">{{ formatDate(obj.pubdate) }}</div>
        </div>
      </van-list>
    </div>

    <!-- 底部添加评论区域 - 1 -->
    <div class="add-cmt-box van-hairline--top" v-if="isShowCmtBox === true">
      <van-icon name="arrow-left" size="0.48rem" @click="$router.back()" />
      <div class="ipt-cmt-div" @click="toggleShowFn">发表评论</div>
      <div class="icon-box">
        <!-- 评论数量 -->
        <van-badge :content="totalCount === 0 ? '' : totalCount" max="99">
          <van-icon
            name="comment-o"
            size="0.53333334rem"
            @click="commentClickFn"
          />
        </van-badge>
        <van-icon name="star" size="0.53333334rem" color="yellow" v-if="artObj.is_collected===true" @click="starClickFn(true)"/>
        <van-icon name="star-o" size="0.53333334rem" v-else @click="starClickFn(false)"/>
        <van-icon name="share-o" size="0.53333334rem" @click="showShare=true"/>
        <van-share-sheet
        v-model="showShare"
        title="立即分享给好友"
        :options="options"
        @select="onSelect"
        get-container="body"
      />
      </div>
    </div>

    <!-- 底部添加评论区域 - 2 -->
    <div class="cmt-box van-hairline--top" v-else>
      <textarea
        v-fofo
        placeholder="友善评论、理性发言、阳光心灵"
        @blur="blurFn"
        v-model.trim="comText"
      ></textarea>
      <!-- 评论输入框中长度大于0时，按钮可用 -->
      <van-button
        type="default"
        :disabled="comText.length === 0"
        @click="sendFn"
        >发布</van-button>

    </div>
  </div>
</template>

<script>
import {
  detailAPI,
  commentsListAPI,
  unLikeCommentAPI,
  likeCommentAPI,
  commentSendAPI,
  unCollectionArticleAPI,
  collectionArticleAPI
} from '@/api/index.js'

import { timeAgo } from '@/utils/date.js'
export default {

  data () {
    return {
      offset: null, // 偏移量评论ID
      commentArr: [], // 评论列表
      totalCount: 0, // 文章评论总数
      isShowCmtBox: true, // 是否显示评论框1
      comText: '', // 评论框输入内容
      loading: false, // 加载状态
      finished: false, // 加载完成
      lastId: null, // 分页
      artObj: {},
      showShare: false, // 分享面板
      options: [
        { name: '微信', icon: 'wechat' },
        { name: '微博', icon: 'weibo' },
        { name: '复制链接', icon: 'link' },
        { name: '分享海报', icon: 'poster' },
        { name: '二维码', icon: 'qrcode' }
      ]
    }
  },
  async created () {
    const res = await commentsListAPI({
      id: this.$route.query.art_id
    })

    this.commentArr = res.data.data.results
    this.totalCount = res.data.data.total_count // 总数量
    this.lastId = res.data.data.last_id // 分页
    // console.log(res)
    // 网页打开 ->没有评论，res结果是空数组
    // 直接让list组件显示没有更多文字
    if (res.data.data.results.length === 0) {
      this.finished = true
    }
  },
  methods: {
    formatDate: timeAgo,
    // 点赞/取消点赞

    async likeClickFn (bool, commentObj) {
      if (bool === true) {
        commentObj.is_liking = false
        const res = await unLikeCommentAPI({
          comId: commentObj.com_id
        })
        console.log(res)
      } else {
        commentObj.is_liking = true
        const res = await likeCommentAPI({
          comId: commentObj.com_id
        })
        console.log(res)
      }
    },
    // 切换评论框
    toggleShowFn () {
      this.isShowCmtBox = false
    },
    // 评论按钮，点击后会滑动到评论页面
    commentClickFn () {
      document.querySelector('.like-box').scrollIntoView({
        behavior: 'smooth'
      })
    },
    // 评论区输入框失去焦点
    blurFn () {
      // 问题：点击发布按钮后发现点击事件不执行---排除代码问题
      // 原因：评论输入框在页面点击发布的一瞬间，失去焦点，被v-if和v-else移除
      // 然后导致点击事件没来得及执行
      // 解决：失去焦点时，变量值在最后再执行
      setTimeout(() => {
        this.isShowCmtBox = true
      })
    },
    // 发布评论
    async sendFn () {
      // 把评论加入到评论列表里
      const res = await commentSendAPI({
        id: this.$route.query.art_id,
        content: this.comText
      })

      this.commentArr.unshift(res.data.data.new_obj)
      this.totalCount++ // 评论总数增加
      this.comText = ''
      this.commentClickFn() // 让第一条评论滑动到屏幕上
    },
    // 加载更多评论
    // 会触发 load 事件并将 loading 设置成 true
    async onLoad () {
      if (this.commentArr.length > 0) {
        // 请求下一页数据
        const res = await commentsListAPI({
          id: this.$route.query.art_id,
          offset: this.lastId
        })

        this.commentArr = [...this.commentArr, ...res.data.data.results]
        this.totalCount = res.data.data.total_count
        this.lastId = res.data.data.last_id // 分页
        if (res.data.data.last_id === null) {
          // 没有下一页了
          this.finished = true
        }
        this.loading = false
        // 4.如果不加immediate-check，上来执行一次onLoad，loading状态为true，但是if又进不去
        // 等created数据回来后撑开高度，滚动到底部时，loading为true就不会重新执行onLoad方法了
        // 效果：一直转圈
        // 解决：if进不去，直接进else
      } else {
        this.loading = false
      }
    },
    // 收藏/取消收藏文章(自己写的，有点乱，但是功能实现了)
    async starClickFn (bool) {
      const res1 = await detailAPI({
        artId: this.$route.query.art_id
      })
      this.artObj = res1.data.data
      if (bool === true) {
        const res2 = await unCollectionArticleAPI({
          artId: this.$route.query.art_id
        })
        this.artObj.is_collected = false
        console.log(res2)
      } else {
        const res2 = await collectionArticleAPI({
          artId: this.$route.query.art_id
        })
        this.artObj.is_collected = true
        console.log(res2)
      }
    },
    // 分享面板
    onSelect () {
      // Toast(option.name)
      this.showShare = false
    }
  }
}

</script>

<style scoped lang="less">
.cmt-list {
  padding: 10px;
  .cmt-item {
    padding: 15px 0;
    + .cmt-item {
      border-top: 1px solid #f8f8f8;
    }
    .cmt-header {
      display: flex;
      align-items: center;
      justify-content: space-between;
      .cmt-header-left {
        display: flex;
        align-items: center;
        .avatar {
          width: 40px;
          height: 40px;
          background-color: #f8f8f8;
          border-radius: 50%;
          margin-right: 15px;
        }
        .uname {
          font-size: 12px;
        }
      }
    }
    .cmt-body {
      font-size: 14px;
      line-height: 28px;
      text-indent: 2em;
      margin-top: 6px;
      word-break: break-all;
    }
    .cmt-footer {
      font-size: 12px;
      color: gray;
      margin-top: 10px;
    }
  }
}
/*美化 - 文章详情 - 底部的评论页面 */
// 外层容器
.art-cmt-container-1 {
  padding-bottom: 46px;
}
.art-cmt-container-2 {
  padding-bottom: 70px;
}

// 发布评论的盒子 - 1
.add-cmt-box {
  position: fixed;
  bottom: 0;
  left: 0;
  width: 100%;
  box-sizing: border-box;
  background-color: white;
  display: flex;
  justify-content: space-between;
  align-items: center;
  height: 46px;
  line-height: 46px;
  padding-left: 10px;
  .ipt-cmt-div {
    flex: 1;
    border: 1px solid #efefef;
    border-radius: 15px;
    height: 30px;
    font-size: 12px;
    line-height: 30px;
    padding-left: 15px;
    margin-left: 10px;
    background-color: #f8f8f8;
  }
  .icon-box {
    width: 40%;
    display: flex;
    justify-content: space-evenly;
    line-height: 0;
  }
}

.child {
  width: 20px;
  height: 20px;
  background: #f2f3f5;
}

// 发布评论的盒子 - 2
.cmt-box {
  position: fixed;
  bottom: 0;
  left: 0;
  width: 100%;
  height: 80px;
  display: flex;
  justify-content: space-between;
  align-items: center;
  font-size: 12px;
  padding-left: 10px;
  box-sizing: border-box;
  background-color: white;
  textarea {
    flex: 1;
    height: 66%;
    border: 1px solid #efefef;
    background-color: #f8f8f8;
    resize: none;
    border-radius: 6px;
    padding: 5px;
  }
  .van-button {
    height: 100%;
    border: none;
  }
}
</style>
